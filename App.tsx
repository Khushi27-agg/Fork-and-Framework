...
const App: React.FC = () => {
  ...
  // Removed showAppShareToast and showAppLinkCopyToast states as sharing is removed
  ...
  const handleToggleSaveRecipe = (recipe: Recipe) => {
    ...
    if (isAlreadySaved) {
      updatedSavedRecipes = savedRecipes.filter(r => r.title !== recipe.title);
    } else {
      updatedSavedRecipes = [...savedRecipes, recipe];
    }
    setSavedRecipes(updatedSavedRecipes);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(updatedSavedRecipes));
  };
  ...
  // Handle shared recipe links on load (keeping this in case user navigation relies on it, but removing UI for creation)
  useEffect(() => {
    const checkHash = () => {
  ...
  // Removed getAppBaseUrl, handleShareApp, handleCopyAppLink functions as sharing is removed

  return (
    <div className="min-h-screen flex flex-col">
       {/* Removed app share/copy toast notifications */}
      <header className="p-6 md:p-8 flex justify-between items-center bg-white/70 backdrop-blur-md sticky top-0 z-30 border-b border-white/20">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-stone-900 leading-none text-serif">Fork & Framework</h1>
            <p className="text-[9px] text-stone-400 font-bold uppercase tracking-[0.3em] mt-1">Where ideas meet flavor</p>
          </div>
        </div>
        
        <div className="flex items-center gap-2 md:gap-4">
          {/* Removed share and copy app link buttons */}
          {profile && mode !== AppMode.Onboarding && (
            <div className="flex items-center gap-4">
               <button onClick={() => setMode(AppMode.Profile)} aria-label="View saved recipes" className="p-2.5 bg-white/50 border border-stone-200 hover:bg-white rounded-2xl transition group">
                <svg className="w-5 h-5 text-stone-500 group-hover:text-emerald-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"></path></svg>
              </button>
              <div className="hidden md:flex items-center gap-4">
                ...
                <button onClick={() => setMode(AppMode.Onboarding)} aria-label="Open user profile settings" className="p-2.5 bg-white/50 border border-stone-200 hover:bg-white rounded-2xl transition group">
      ...
      <main className="px-6 md:px-8 max-w-7xl mx-auto py-8 flex-1">
        ...
        {mode === AppMode.Dashboard && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div className="lg:col-span-7 space-y-8">
              <section aria-labelledby="pantry-heading" className="bg-white/80 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-xl border border-white/40">
                <h2 id="pantry-heading" className="text-2xl font-bold mb-2 text-stone-800">What's in your pantry?</h2>
                <p className="text-stone-400 text-sm mb-6">Add ingredients manually or snap a quick photo</p>
                
                <div className="space-y-6">
                  <div className="relative group">
                    <label htmlFor="ingredients-textarea" className="sr-only">List your ingredients</label>
                    <textarea
                  ...
                  <div className="relative">
                    <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-stone-200 rounded-[2rem] cursor-pointer hover:border-emerald-400 hover:bg-emerald-50/30 transition-all duration-300 group bg-white/30">
                      ...
                      {isAnalyzingImage ? (
                        <div role="status" className="flex flex-col items-center animate-in zoom-in-95">
                          <div className="w-10 h-10 border-4 border-emerald-600 border-t-transparent rounded-full animate-spin mb-3" aria-hidden="true"></div>
                          <span className="text-emerald-700 font-bold text-sm">Visual Scan in Progress...</span>
                        </div>
                      ) : (
                        <>
                          <div className="w-12 h-12 bg-white/80 rounded-2xl flex items-center justify-center text-stone-400 group-hover:bg-emerald-100 group-hover:text-emerald-600 transition-colors mb-2 shadow-sm" aria-hidden="true">
              ...
              <fieldset className="bg-white/80 backdrop-blur-xl p-8 rounded-[2.5rem] shadow-xl border border-white/40">
                <legend className="text-2xl font-bold mb-2 text-stone-800">Choose your Cook Mood</legend>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {MOODS.map((mood) => (
                    ...
                    >
                      <span className="text-3xl mb-3" aria-hidden="true">{mood.icon}</span>
                      ...
                      {selectedMood === mood.id && (
                        <div className="absolute top-4 right-4 text-emerald-500" aria-hidden="true">
            ...
            <div className="lg:col-span-5 flex flex-col gap-8">
              <section aria-labelledby="selected-items-heading" className="bg-stone-900/95 backdrop-blur-2xl p-8 rounded-[2.5rem] shadow-2xl flex-1 flex flex-col relative overflow-hidden border border-stone-800">
                <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl" aria-hidden="true"></div>
                
                <h2 id="selected-items-heading" className="text-2xl font-bold mb-2 text-white flex justify-between items-center">
                  Selected Items
                ...
                <div className="flex-1 mt-6 overflow-y-auto max-h-[400px] pr-2 custom-scrollbar">
                  {analyzedIngredients.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-stone-600 text-center py-12">
                      <div className="w-16 h-16 bg-stone-800/50 rounded-3xl flex items-center justify-center mb-4 border border-stone-700" aria-hidden="true">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {analyzedIngredients.map((ing) => (
                        <div key={ing} className="bg-stone-800 px-4 py-2.5 rounded-2xl flex items-center gap-3 border border-stone-700 animate-in zoom-in-90 duration-300">
                          ...
                          <button onClick={() => removeIngredient(ing)} aria-label={`Remove ${ing}`} className="text-stone-500 hover:text-rose-400 transition-colors">
                ...
                >
                  {isGenerating ? (
                    <>
                      <div role="status" className="w-6 h-6 border-3 border-white border-t-transparent rounded-full animate-spin">
                    ...
                    <>
                      <span className="text-2xl" aria-hidden="true">{MOODS.find(m => m.id === selectedMood)?.icon}</span>
                      <span className="tracking-wide">Generate Healthy Recipe</span>
                    </>
                  )}
                </button>
              </section>
        ...
        {mode === AppMode.Profile && (
          <ProfileView
            ...
            onUnsaveRecipe={(title) => handleToggleSaveRecipe({title} as Recipe)}
            onBack={handleBackToDashboard}
          />
        )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex gap-4">
                 ...
                 <a href="#" aria-label="X (formerly Twitter)" className="hover:text-emerald-400 transition-colors">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 text-sm">
                <div className="flex items-start gap-3">
                  <svg className="w-5 h-5 text-emerald-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
